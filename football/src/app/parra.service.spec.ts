...
fdescribe('ParraService', () => {
  ...
  it('should fetchplayers by keyvalue', (inject([ParraService], (parraService, mockBackend) => {
    ...
    players = parraService.filterPlayersByKeyValueNullCheck(kv);
    expect(players.length).toEqual(2);
    kv = { key: 'age', value: '27' };
    //    players = parraService.filterPlayersByKeyValue({ key: 'age', value: '27' });
    players = parraService.filterPlayersByKeyValueNullCheck({ key: 'age', value: '27' });
    expect(players.length).toEqual(1);
  })));


  it('should fetchplayers with stats', (inject([ParraService], (parraService, mockBackend) => {
    let players: Player[] = []
    players = parraService.filterPlayersWithStats({ key: 'position', value: 'prop' });
  ...
  it('should fetchplayersX by keyvalue', (inject([ParraService], (parraService, mockBackend) => {
    let players: Player[] = []
    let kv: KeyValuePair = { key: 'position', value: 'prop' };
    players = parraService.filterPlayersByKeyValueX(kv);
    expect(players.length).toEqual(2);
    kv = { key: 'age', value: '27' };
